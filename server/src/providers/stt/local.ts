...
export const localSttProvider: SpeechToTextProvider = {
  async transcribeSpeech(input: SttRequest): Promise<string> {
    if (!input.provider.baseUrl) throw new Error('Local STT requires baseUrl');
    ...
    if (!response.ok) throw new Error(`Local STT failed ${response.status}`);
