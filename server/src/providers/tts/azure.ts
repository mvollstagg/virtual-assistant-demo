...
export const azureTtsProvider: TextToSpeechProvider = {
  async synthesizeSpeech(input: TtsRequest) {
    ...
    if (!key || !region) throw new Error('Azure TTS requires key and region (use baseUrl as region in settings)');

    return await new Promise((resolve, reject) => {
      const speechConfig = sdk.SpeechConfig.fromSubscription(key, region);
      speechConfig.speechSynthesisOutputFormat = sdk.SpeechSynthesisOutputFormat.Audio16Khz32KBitRateMonoMp3;
      if (input.voice) speechConfig.speechSynthesisVoiceName = input.voice;

      const blendData: Array<{ time: number; blendshapes: Record<string, number> }> = [];
      ...
      synthesizer.speakSsmlAsync(
        SSML.replace('__TEXT__', input.text),
        () => {
          synthesizer.close();
